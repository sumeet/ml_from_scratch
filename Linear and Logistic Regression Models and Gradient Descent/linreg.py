...
def log_likelihood(X, y, B,lmbda):
    return -np.sum(np.dot(y,np.dot(X,B)) + np.log(1 + np.exp(X, B)))
...
def minimize(X, y, loss_gradient,
    ...
    iter = 0
    while iter <= max_iter:
        loss_g = loss_gradient(X, y, B, lmbda)
        if LA.norm(loss_g) <= precision:
            break
        h += np.multiply(loss_g,loss_g)
        # for i in range(p):
        #     B[i] = B[i] - (eta/np.sqrt(h[i]) + eps) * loss_g[i]
        B = B - (eta/np.sqrt(h) + eps) * loss_g
        iter += 1
    return B
...
class LogisticRegression621: # MODIFY THIS ONE
    ...
    def predict(self, X):
        ...
        B0 = np.ones(shape=(n, 1))
        X = np.hstack([B0, X])
        proba = sigmoid(np.dot(X, self.B))
        return np.array([1 if i >0.5 else 0 for i in proba])
